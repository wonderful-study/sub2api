...
type OpenAITokenInfo struct {
	...
	// SubscriptionExpiresAtChecked is true when accounts/check returned account info.
	// It lets credential writes clear a stale subscription_expires_at when upstream
	// now reports no paid entitlement expiration.
	SubscriptionExpiresAtChecked bool   `json:"-"`
...
func (s *OpenAIOAuthService) enrichTokenInfo(ctx context.Context, tokenInfo *OpenAITokenInfo, proxyURL string) {
	...
	if info := fetchChatGPTAccountInfo(ctx, s.privacyClientFactory, tokenInfo.AccessToken, proxyURL, orgID); info != nil {
		tokenInfo.SubscriptionExpiresAtChecked = true
		if info.PlanType != "" {
...
func (s *OpenAIOAuthService) BuildAccountCredentials(tokenInfo *OpenAITokenInfo) map[string]any {
	...
	if strings.EqualFold(tokenInfo.PlanType, "free") {
		creds["subscription_expires_at"] = ""
	} else if tokenInfo.SubscriptionExpiresAtChecked {
		creds["subscription_expires_at"] = tokenInfo.SubscriptionExpiresAt
	} else if tokenInfo.SubscriptionExpiresAt != "" {
		creds["subscription_expires_at"] = tokenInfo.SubscriptionExpiresAt
	}
